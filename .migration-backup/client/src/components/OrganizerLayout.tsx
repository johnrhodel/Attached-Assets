...
export function OrganizerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: "/organizer/dashboard", label: t.organizer?.nav?.dashboard || t.nav.dashboard, icon: LayoutDashboard },
  ];
    ...
    <div className="flex flex-col h-full">
      <div className="px-6 py-6 flex items-center gap-3">
        ...
        <div>
          ...
          <span className="text-xs text-muted-foreground">{t.organizer?.title || "Organizer"}</span>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-border space-y-3">
        {user && (
          <div className="px-4 py-2">
            <p className="text-sm font-medium truncate" data-testid="text-organizer-user-name">{user.name || user.email}</p>
            <p className="text-xs text-muted-foreground truncate" data-testid="text-organizer-user-email">{user.email}</p>
          </div>
        )}
        <div className="px-2">
          <LanguageSelector />
          ...
          <div className="flex items-center gap-2">
            ...
            <Sheet open={open} onOpenChange={setOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" data-testid="button-organizer-mobile-menu">
