...
NFTs are minted on the Solana devnet using Metaplex Core. The system uses a persistent server keypair loaded from `SOLANA_KEYPAIR_JSON` (preferred — accepts a JSON array of 64 integers), with `SOLANA_SERVER_SECRET_KEY` (base58 string) and `STELLAR_SERVER_SECRET_KEY` as fallbacks. Dynamic NFT metadata is served via a dedicated API endpoint to ensure proper display in Solana explorers and wallets. Custodial wallets use AES-256-CBC encryption. The JSON-array format is preferred because it is immune to invisible-character corruption that can happen when pasting base58 strings into Secrets panels — any tampering breaks JSON parse explicitly instead of silently producing a wrong key.
